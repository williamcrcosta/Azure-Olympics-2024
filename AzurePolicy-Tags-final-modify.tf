...
resource "azurerm_subscription_policy_remediation" "remediate_add_tag_policy" {
  name                    = "remediate-add-tag-policy"
  subscription_id         = data.azurerm_subscription.current.id
  policy_assignment_id    = azurerm_subscription_policy_assignment.assign_add_tag_policy.id
  resource_discovery_mode = "ReEvaluateCompliance"
  depends_on = [
    azurerm_role_assignment.assign_role_to_policy_identity
  ]
}
